...
public class Robot extends TimedRobot {
  ...
  PIDController balancingVelocityPIDControllers = new PIDController(var.balancing_kp, var.balancing_ki, var.balancing_kd);
  ...
  public void robotInit() {
    ...
    m_chooser.addOption("Right Auto", kSideAuto);
  ...
  public void autoWinchOutTake(){
    double winchStartTime = 0; 
    ...
    // out take preload
      ...
      if (System.currentTimeMillis() > (winchStartTime + 5000)) {
        autoWinchMode++;
      }
    }

    // bring winch in
    else if (autoWinchMode == 3) {
      winchMode = 2;
    }
  ...
  public void balanceOnDock() {
    ...
    if(Math.abs(gyroPitch - gyroSetPoint) < 0.5){
      autonStage++; 
    } 

    double dockVelocity = 200; 
    double calculatedBalanceSpeed = gryoPitchPIDControllers.calculate(gyroPitch, pitchSetpoint);

    // will be getting a setpoing up to 200 
    double rightSideSetpointVelocity; 
    double leftSideSetpointVelocity; 

    double rightSideFinalBalancingVelocity; 
    double leftSideFinalBalancingVelocity; 

    if (calculatedBalanceSpeed > 0) {
      rightSideSetpointVelocity = dockVelocity * calculatedBalanceSpeed; 
      leftSideSetpointVelocity = dockVelocity * calculatedBalanceSpeed; 
    }

    else if(calculatedBalanceSpeed < 0){
      rightSideSetpointVelocity = -1*(dockVelocity * calculatedBalanceSpeed); 
      leftSideSetpointVelocity = -1*(dockVelocity * calculatedBalanceSpeed); 
    }

    else{
      rightSideSetpointVelocity = 0; 
      leftSideSetpointVelocity = 0; 
    } 

    leftSideFinalBalancingVelocity = balancingVelocityPIDControllers.calculate(leftFrontEncoder.getVelocity(), rightSideSetpointVelocity); 
    rightSideFinalBalancingVelocity = balancingVelocityPIDControllers.calculate(rightFrontEncoder.getVelocity(), leftSideSetpointVelocity); 


    double maxOutput = 0.8;

    if (leftSideFinalBalancingVelocity > maxOutput) {
      leftSideFinalBalancingVelocity = maxOutput;
    }

    else if (leftSideFinalBalancingVelocity < -maxOutput) {
      leftSideFinalBalancingVelocity = -maxOutput;
    }

    if (rightSideFinalBalancingVelocity > maxOutput) {
      rightSideFinalBalancingVelocity = maxOutput;
    }

    else if (rightSideFinalBalancingVelocity < -maxOutput) {
      rightSideFinalBalancingVelocity = -maxOutput;
    }

    leftMotorFront.set(leftSideFinalBalancingVelocity);
    rightMotorBack.set(rightSideFinalBalancingVelocity);
  }
